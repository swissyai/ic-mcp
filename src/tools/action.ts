...
export const actionTool = {
  name: 'icp/action',
  description: `Execute ICP code operations with natural language commands.

UNIFIED INTERFACE for all code operations: validation, testing, deployment, refactoring, analysis, and upgrade checks.

Natural language examples:
- "validate my Motoko code" → code validation
- "test the transfer method" → execute canister method
- "deploy to local network" → deploy canisters
- "refactor to add upgrade hooks" → code transformation
- "analyze project structure" → project analysis
- "check if interface is upgrade-safe" → upgrade validation

Features:
- Intent parsing with confidence scoring
- Parameter extraction from natural language
- Routes to specialized validators/executors
- TOON format by default (50% fewer tokens)
- Comprehensive error handling with examples

Context parameters:
- code: Source code to validate/refactor
- language: motoko, rust, candid, dfx-json
- canisterId, method, args: For testing
- projectPath, network, mode: For deployment
- oldCandid, newCandid: For upgrade checks`,
