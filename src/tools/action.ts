...
export const actionTool = {
  name: 'icp/action',
  description: `ICP code operations - validates, tests, deploys, refactors, analyzes ICP code.

PURPOSE: Execute development operations on ICP code with natural language commands.

WHEN TO USE:
- User wants to validate code (Motoko, Rust, Candid, dfx.json)
- User needs to test canister methods
- User wants to deploy to local/playground
- User asks to refactor code (add hooks, stable vars, auth)
- User needs project analysis
- User checks upgrade safety

6 ACTION TYPES:

1. VALIDATE (validate, check, verify, lint)
   Purpose: Check code syntax and semantics with real compilers
   Examples:
   - "validate my Motoko code" + context: {code, language: "motoko"}
   - "check this Candid interface" + context: {code, language: "candid"}
   - "verify dfx.json" + context: {code, language: "dfx-json"}

   Required: code, language
   Compilers used: moc (Motoko), didc (Candid), pattern checks (Rust)
   Returns: {valid: bool, issues: [{severity, message, line, suggestion}]}

2. TEST (test, call, invoke, execute, run)
   Purpose: Execute canister methods with proper Candid encoding
   Examples:
   - "test the transfer method" + context: {canisterId, method, args}
   - "call greet function" + context: {canisterId: "abc123", method: "greet"}
   - "execute mint with 1000 tokens" + context: {canisterId, method: "mint", args: [1000]}

   Required: canisterId, method
   Optional: args, network (local/playground/ic), mode (query/update), projectPath
   Returns: method execution result or error

3. DEPLOY (deploy, install, publish)
   Purpose: Deploy canisters to local or playground networks
   Examples:
   - "deploy to local network"
   - "install canisters with clean build"
   - "upgrade my canisters" + context: {mode: "upgrade"}

   Optional: projectPath (defaults to cwd), network (local/playground),
            canisters (specific ones), mode (install/reinstall/upgrade),
            clean (default: true)
   Returns: deployment status, canister IDs, build output

4. REFACTOR (refactor, transform, add, modernize)
   Purpose: Apply ICP-specific code transformations
   Examples:
   - "add upgrade hooks" + context: {code, language: "motoko"}
   - "convert to stable storage" + context: {code, language, refactoring: "add-stable-vars"}
   - "add caller authentication" + context: {code, language, refactoring: "add-caller-checks"}

   Required: code, language (motoko/rust), refactoring type
   Types: add-upgrade-hooks, add-stable-vars, add-caller-checks, modernize
   Returns: {refactored: code, changes: [{type, description, linesAdded, linesModified}]}

5. ANALYZE (analyze, inspect, review, examine)
   Purpose: Analyze project structure, dependencies, circular deps
   Examples:
   - "analyze project structure"
   - "check for circular dependencies"
   - "validate all canisters" + context: {validate: true}

   Optional: projectPath (defaults to cwd), validate (default: true),
            checkDependencies (default: true)
   Returns: {canisters, loc, dependencies, circularDeps, buildOrder, issues}

6. CHECK-UPGRADE (check, verify upgrade compatibility)
   Purpose: Verify Candid interface upgrade safety
   Examples:
   - "check if interface is upgrade-safe" + context: {oldCandid, newCandid}
   - "verify upgrade compatibility" + context: {old: "service {...}", new: "service {...}"}

   Required: oldCandid, newCandid (Candid interface definitions)
   Returns: {safe: bool, breaking: [], warnings: [], details}
...
ERROR HANDLING:
All actions return helpful errors with:
- What went wrong
...
GUIDANCE FOR USERS:
- Validation: "Provide code in context: {code, language}"
- Testing: "Need canisterId and method name"
- Deployment: "Run from project directory or provide projectPath"
- Refactoring: "Specify refactoring type: add-upgrade-hooks, add-stable-vars, etc."
- Analysis: "Run from project root or provide projectPath"
- Upgrade check: "Provide both old and new Candid interfaces"
...
- If validation fails, show them the specific errors with line numbers
